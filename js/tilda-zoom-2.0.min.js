...
function t_initZoom() {
  ...
  if (!document.querySelector(".t-zoomer__wrapper")) {
    ...
    var zoomableElements = document.querySelectorAll("[data-zoomable]"),
      zoomableElementsNotEmpty = Array.prototype.slice
        .call(zoomableElements)
        .filter(function (el) {
          return (
            "yes" === el.getAttribute("data-zoomable") &&
            !el.classList.contains("t-slds__thumbs_gallery") &&
            "" !== el.getAttribute("data-img-zoom-url")
          );
        });
    Array.prototype.forEach.call(
      zoomableElementsNotEmpty,
      function (zoomableEl) {
    ...
    var zoomCloseBtn = t_zoom__createCloseBtn(),
      scaleBtn = t_zoom__createScaleBtn();
...
function t_zoom__createScaleBtn() {
  ...
  scaleBtn.classList.add("t-zoomer__scale"), scaleBtn.classList.add("showed");
  var svgScaleBtn = "";
  return (
    (svgScaleBtn +=
...
function t_zoom__initFullScreenImgOnClick() {
  document.addEventListener("click", function (e) {
    var zoomableElement = e.target.closest(
      '.t-zoomable:not([data-img-zoom-url=""]), .t-slds__thumbs_gallery-zoomable'
    );
    zoomableElement && t_zoomHandler(zoomableElement);
  });
...
function t_zoomHandler(targetElement) {
  ...
  var zoomContainer = document.querySelector(".t-zoomer__container"),
    zoomerCarousel = document.createElement("div");
  ...
  var leftArrow = t_zoom_createSliderArrow("left"),
    rightArrow = t_zoom_createSliderArrow("right");
  zoomerSlide.appendChild(leftArrow),
    ...
    zoomContainer && (zoomContainer.innerHTML = ""),
    zoomContainer && zoomContainer.appendChild(zoomerCarousel);
  var parentBlock = targetElement.closest(".r"),
    sliderTrack;
  if (!document.querySelector(".t-carousel__zoomer__track") || !parentBlock)
    return !1;
...
function t_zoom__updateActiveSlidePos() {
  setTimeout(function () {
    ...
    if (sliderTrack) {
      var activeSlidePosition = sliderTrack.querySelector(
        ".t-carousel__zoomer__item.active"
      ).offsetLeft;
      sliderTrack.style.transform =
        "translateX(" + -activeSlidePosition + "px)";
    }
  }, 300);
}
function t_zoom_createSliderArrow(direction) {
  ...
  ArrowWrapper.classList.add("t-carousel__zoomer__arrow__wrapper"),
    ArrowWrapper.classList.add(
      "t-carousel__zoomer__arrow__wrapper_" + direction
    );
...
function t_zoom__initEventsonMobile() {
  if (window.t_zoom__isMobile) {
    ...
    var modal = document.querySelector(".t-zoomer__wrapper"),
      modalEvents;
    ["touchstart", "touchmove", "touchend", "mousemove"].forEach(function (
      event
    ) {
...
function t_zoom__updateSlidesHeight() {
  var slideImageList = document.querySelectorAll(
      ".t-carousel__zoomer__item .t-carousel__zoomer__img"
    ),
    modal = document.querySelector(".t-zoomer__wrapper");
  if (modal && slideImageList.length) {
    var commentHeight = modal.getAttribute("data-max-comment-height");
    (commentHeight = parseInt(commentHeight, 10)),
      isNaN(commentHeight) ||
        setTimeout(function () {
          var zoomedItem = document.querySelector(
            ".t-carousel__zoomer__item.active"
          );
          if (zoomedItem) {
            var imageBordersWidth,
              imagePadding =
                modal.offsetHeight - zoomedItem.offsetHeight + commentHeight,
              imageHeight =
                document.documentElement.clientHeight - imagePadding;
...
function t_zoom__addingImgsIntoCarousel(targetElement) {
  var parentBlock = targetElement.closest(".r"),
    images = parentBlock
      ...
      : [],
    slide;
  if (parentBlock ? parentBlock.querySelector(".t-slds") : null) {
    ...
    slider &&
      ...
      slider &&
        slider.querySelector(".tn-elem__gallery__video-wrapper") &&
        (images = Array.prototype.slice.call(images)).splice(-1, 1);
  }
  var fontFamilyObj = t_zoom__getZoomDescriptionFontFamily(parentBlock),
  ...
  Array.prototype.forEach.call(images, function (img, i) {
    ...
    if (
      ...
      slideTitle.classList.add("t-zoomer__title"),
        slideTitle.classList.add("t-descr"),
        slideTitle.classList.add("t-descr_xxs"),
        (slideTitle.textContent = imgTitle),
        fontFamilyObj &&
          fontFamilyObj.titleFontFamily &&
          (slideTitle.style.fontFamily = fontFamilyObj.titleFontFamily);
    }
    if (imgDescr) {
    ...
    if (
      ...
      isLazy
        ? (slideIMG.classList.add("t-img"),
          ...
          (0 !== i && i !== images.length - 1) ||
            slideIMG.setAttribute("data-lazy-rule", "skip"))
        : (slideIMG.src = imgURLs),
      zoomerSlide.appendChild(zoomerWrapper),
      zoomerWrapper.appendChild(slideIMG),
      imgTitle || imgDescr || window.t_zoom__isMobile)
    ) {
      var slideComment = document.createElement("div");
      slideComment.classList.add("t-zoomer__comments"),
        imgTitle && slideComment.appendChild(slideTitle),
        imgDescr && slideComment.appendChild(slideDescription),
        zoomerSlide.appendChild(slideComment);
    }
    sliderTrack.appendChild(zoomerSlide);
  });
}
function t_zoom__getZoomDescriptionFontFamily(parentBlock) {
  var titleBlock = parentBlock.querySelector(
      '.t827__overlay-title, .t979__overlay-title, .t-slds__title, .t603__title, [itemprop="name"]'
    ),
    descrBlock = parentBlock.querySelector(
  ...
  return {
    titleFontFamily:
      (titleBlock && getComputedStyle(titleBlock).fontFamily) || "",
    descrFontFamily:
      (descrBlock && getComputedStyle(descrBlock).fontFamily) || "",
  };
}
  ...
  if (modal && slideItems.length) {
      ...
      var maxHeightOfComments = Array.prototype.reduce.call(
        ...
        function (prev, slide) {
          var comment = slide.querySelector(".t-zoomer__comments"),
            commentHeight = comment ? comment.offsetHeight : 0;
          return commentHeight > prev && (prev = commentHeight), prev;
        },
        0
      );
      modal.setAttribute("data-max-comment-height", maxHeightOfComments);
    }
    var isLazy = "y" === window.lazy;
    Array.prototype.forEach.call(slideItems, function (slideItem) {
      var image = slideItem.querySelector(".t-carousel__zoomer__img"),
        attributeName = isLazy ? "data-original" : "src",
        imageSrc = image.getAttribute(attributeName),
        comment = slideItem.querySelector(".t-zoomer__comments"),
        commentHeight = comment ? comment.offsetHeight : 0;
      window.t_zoom__isMobile && (commentHeight = maxHeightOfComments);
      var imagePadding = imageBordersWidth + commentHeight;
      comment && (comment.style.height = commentHeight + "px"),
        (image.style.maxHeight =
          document.documentElement.clientHeight - imagePadding + "px"),
        imageSrc &&
          -1 !== imageSrc.indexOf(".svg") &&
      ...
      Array.prototype.forEach.call(arrowWrappers, function (arrowWrapper) {
        arrowWrapper.style.top = commentHeight
          ? "calc(50% - " + commentHeight / 2 + "px)"
          : "50%";
      });
    }),
    ...
    var targetURL = targetElement.getAttribute("data-img-zoom-url"),
      ...
      selector =
        ...
        (isLazy ? "data-original" : "src") +
        '="' +
        targetURL +
        '"]',
      targetImg = targetURL ? document.querySelector(selector) : null,
      carouselItem = targetImg
        ? targetImg.closest(".t-carousel__zoomer__item")
        : null;
    if (carouselItem) {
      var carouselPosLeft = !!carouselItem && carouselItem.offsetLeft;
      carouselItem.classList.add("active"),
        t_zoom__hideInnactiveSlides(carouselItem, sliderTrack),
        (sliderTrack.style.transition = "none"),
        (sliderTrack.style.transform =
          "translateX(" + -carouselPosLeft + "px)"),
        setTimeout(function () {
          sliderTrack.style.transition = "";
        }, 200);
    }
    isLazy && t_onFuncLoad("t_lazyload_update", t_lazyload_update);
  }
}
function t_zoom__hideInnactiveSlides(activeSlide, sliderTrack) {
  if (activeSlide) {
    var currentSlidesSelector =
        ".t-carousel__zoomer__item:not(:first-child):not(:last-child)",
      slides = Array.prototype.slice.call(
        sliderTrack.querySelectorAll(currentSlidesSelector)
      );
    if (!(slides.length <= 6)) {
      var activeSlideIndex = t_zoom__getCurrentSlideIndex(activeSlide),
        activeIndexList = [activeSlideIndex];
      activeIndexList.push(
        0 === activeSlideIndex ? slides.length : activeSlideIndex - 1
      ),
        activeIndexList.push(
          activeSlideIndex === slides.length ? 0 : activeSlideIndex + 1
        );
      var innactiveClass = "t-carousel__zoomer__item-innactive";
      slides.forEach(function (slide) {
        var currentSlideIndex = t_zoom__getCurrentSlideIndex(slide),
          currentSlideOperation =
            -1 !== activeIndexList.indexOf(currentSlideIndex)
              ? "remove"
              : "add";
        slide.classList[currentSlideOperation](innactiveClass);
      });
    }
  }
}
function t_zoom__getCurrentSlideIndex(slide) {
  return (
    (slide && parseInt(slide.getAttribute("data-zoomer-slide-number"), 10)) || 0
  );
}
function t_zoom_showSlide(direction) {
  var sliderTrack = document.querySelector(".t-carousel__zoomer__track"),
    ...
    currentSlideIndex = 0,
    isClickFromZoomedImg = "y" === sliderTrack.getAttribute("data-cached-zoom");
  if (
    (Array.prototype.forEach.call(slideItems, function (slideItem, index) {
    ...
    var slideCount =
      "next" === direction
        ? currentSlideIndex + 1
        : slideItems.length + (currentSlideIndex - 1);
    (currentSlideIndex = slideCount % slideItems.length),
      sliderTrack.setAttribute("data-on-transition", "y"),
      window.t_zoom__isMobile &&
        "0s" === getComputedStyle(sliderTrack).transitionDuration &&
        !isClickFromZoomedImg &&
        (sliderTrack.style.transition = "");
  }
  ...
  if (
    ...
    t_zoom__hideInnactiveSlides(slideItems[currentSlideIndex], sliderTrack),
    isClickFromZoomedImg)
  ) {
...
function t_zoom_loopSlider() {
  ...
  var slidesCount = (sliderItems = sliderTrack.querySelectorAll(
    ...
    events;
  ["transitionend", "webkitTransitionEnd", "oTransitionEnd"].forEach(function (
    event
  ) {
...
function t_zoom__updateClonedImgSrc(
  firstSlide,
  lastSlide,
  firstSlideClone,
  lastSlideClone
  ...
  if ("y" === window.lazy) {
    var firstSlideIMG = firstSlide.querySelector("img"),
      lastSlideIMG = lastSlide.querySelector("img"),
      firstSlideCloneIMG = firstSlideClone.querySelector("img"),
      lastSlideCloneIMG = lastSlideClone.querySelector("img"),
      listOfClonedEls = [
        firstSlideIMG,
        lastSlideIMG,
        firstSlideCloneIMG,
    ...
    if (
      !listOfClonedEls.some(function (el) {
        return !el;
      }) &&
      ...
      var mutationObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          ...
          "attributes" === mutation.type &&
            ...
            (mutation.target === firstSlideIMG &&
              (updatedEl = firstSlideCloneIMG),
            mutation.target === lastSlideIMG && (updatedEl = lastSlideCloneIMG),
            mutation.target === firstSlideCloneIMG &&
              (updatedEl = firstSlideIMG),
            mutation.target === lastSlideCloneIMG && (updatedEl = lastSlideIMG),
            mutation.target.src &&
              !updatedEl.src &&
              (updatedEl.src = mutation.target.src));
        });
      });
      listOfClonedEls.forEach(function (el) {
        mutationObserver.observe(el, { attributes: !0 });
      });
    }
  }
}
...
function t_zoom_checkForScale() {
  ...
  if (zoomedImage) {
    var allRecords = document.getElementById("allrecords"),
      isLazyActive =
        !!allRecords && "yes" === allRecords.getAttribute("data-tilda-lazy");
    if ("y" === window.lazy || isLazyActive) {
      var timer = Date.now();
      t_zoom__waitImgForScale(zoomedImage, timer, function () {
        t_zoom_checkToScaleInit(zoomedImage);
      });
    } else
      zoomedImage.complete
        ? t_zoom_checkToScaleInit(zoomedImage)
...
function t_zoom__waitImgForScale(zoomedImage, timer, callback) {
  zoomedImage.src && zoomedImage.naturalWidth && zoomedImage.naturalHeight
    ? callback()
    : Date.now() - timer < 3e3
    ? setTimeout(function () {
...
function t_zoom_checkToScaleInit(zoomedImage) {
  ...
  var hasOriginalSvgParams =
    zoomedImage.hasAttribute("data-original-svg-height") ||
    zoomedImage.hasAttribute("data-original-svg-width");
  -1 === zoomedImage.src.indexOf(".svg") || window.isIE || hasOriginalSvgParams
...
function t_zoom_fetchSVG(zoomedImage, windowHeight, windowWidth) {
  ...
  fetch(url)
    ...
    .then(function (svg) {
      ...
      zoomedImage.setAttribute(
        "data-original-svg-height",
        parsedSVG.getBBox().height.toString()
      ),
        zoomedImage.setAttribute(
          "data-original-svg-width",
          parsedSVG.getBBox().width.toString()
        ),
        (zoomedImage.style.width = parsedSVG.getBBox().width + "px"),
        (zoomedImage.style.height = parsedSVG.getBBox().height + "px"),
        windowHeight < parsedSVG.getBBox().height ||
        windowWidth < parsedSVG.getBBox().width
          ? (!window.isDoubletapScaleAdded &&
              window.t_zoom__isMobile &&
              t_zoom_doubletapScaleInit(),
            t_zoom_scale_init())
      ...
      zoomerWrapper.addEventListener(
        ...
        function (e) {
          var zoomedImage = document.querySelector(
            ...
            noScaleCurrentIMG =
              !zoomerWrapper.classList.contains("zoomer-no-scale");
          ((window.t_zoom__isMobile &&
...
function t_zoom_desktopZoomPositioningInit(zoomedImage, event) {
  var leftCoord = (window.innerWidth - zoomedImage.offsetWidth) / 2,
    ...
    imageXpx,
    naturalWidth =
      zoomedImage.getAttribute("data-original-svg-width") ||
      zoomedImage.naturalWidth,
    naturalHeight =
      zoomedImage.getAttribute("data-original-svg-height") ||
      zoomedImage.naturalHeight;
  ...
  (zoomedImage.style.left = leftCoord + "px"),
    ...
    window.innerWidth < naturalWidth && window.innerHeight < naturalHeight
      ? ((clientXpercent = (100 * event.clientX) / window.innerWidth),
        (imageXpx =
          (-clientXpercent * (naturalWidth - window.innerWidth)) / 100),
        (clientYpercent = (100 * event.clientY) / window.innerHeight),
        (imageYpx =
          (-clientYpercent * (naturalHeight - window.innerHeight)) / 100),
  ...
  (zoomedImage.ontouchstart = function (e) {
    ...
    (zoomedImage.ontouchmove = function (e) {
      ...
      (currentTranslate.x = currentPosition.x + imageOffsetX),
        ...
        currentTranslate.y < topCoordinate &&
          (currentTranslate.y = topCoordinate),
        zoomedImage.offsetHeight < window.innerHeight &&
...
function t_zoom_unscale() {
  ...
  if (zoomedItem) {
    var image = zoomedItem.querySelector(".t-carousel__zoomer__img"),
      comment = zoomedItem.querySelector(".t-zoomer__comments");
    if (modal) {
      var imageBordersWidth = modal.offsetHeight - zoomedItem.offsetHeight,
        commentHeight = comment ? comment.offsetHeight : 0;
      window.t_zoom__isMobile &&
        (commentHeight =
          modal.getAttribute("data-max-comment-height") || commentHeight);
      var imagePadding = imageBordersWidth + commentHeight;
  ...
  scaledZoomerWrappers.length &&
    zoomerTrack.setAttribute("data-cached-zoom", "y"),
    setTimeout(function () {
